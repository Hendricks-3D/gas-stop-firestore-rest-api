...
const addUser = async (req: Request,res:Response)=>{
    ...
    const{name,password,email,carType} = req.body;
    try{
        if(name&&password&&email){
            const user = db.collection('users').doc();//Create a user collection in firestore
                ...
                const userObject = {
                    id:user.id,//get the id from the newly created collection
                    name,
                    password,
                    email,
                    carType,
...
const updateUser = async (req:Request, res:Response)=>{
    const{body: {name,password,email,carType},params:{userId}} = req;//Creating and Object of type user and assign the request to it
    try{
